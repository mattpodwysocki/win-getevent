...
class WinEventEmitter extends EventEmitter {
  ...
  start () {
    this._id = setTimeout(() => {
      ...
      const getAllLogs = (this.options.providers && this.options.providers.length > 0 && this.options.providers[0]=='*')
      if (!getAllLogs) {
      ...
      if (getAllLogs) {
        powershellCmd = `powershell "${creds}Get-WinEvent ---ListLog  *; StartTime='${startTime}'; EndTime='${endTime}';} -MaxEvents ${this.options.maxEvents}${credsOpt}${computerOpt} | ConvertTo-Json`;
      }
      ...
      // Ensure no time gaps
      var now = Date.now()
      this.options.startTime = new Date(this.options.endTime || now)
